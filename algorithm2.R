...
#test
test=OrderKmeans(X)
OrderKmeans=function(X,M_max=4,penalty=log(N),num_min=2,n_init=3){
    N=dim(X)[1];D=dim(X)[2]
    wgss_list=list()
    for (j in 1:n_init) {
        #test
    ...
    m_changepints=list(M_hat=global_num,changepoints_hat=global_changePoints)
    return(m_changepints)
}
traceback()
